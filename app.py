...
def gatherizer_tab():
    ...
    for question_people in unique_questions:
        ...
        profile_type = question_df[question_df.answer == answer_people].profile_type.values
        df = pd.DataFrame({'nom': [nom], 'prenom': [prenom], 'mail': [mail],'question': [question_people], 'answer': [answer_people],'score': [score],'profile_type':[profile_type]})
